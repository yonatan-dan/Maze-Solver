...
public class UnionFind { 
   ...
   public void union (int i, int j) { 
      // check if i are not representative elements
      if(up[i] != -1 || up[j] != -1) {
         throw new IllegalArgumentException("i and j must be the representatives of their sets");
      }
      if(i == j) return; // check if i and j are in the same set

      // unites the sets using weighted union
